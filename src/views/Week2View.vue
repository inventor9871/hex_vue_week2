    ...
    <div class="row">
      ...
      <div class="col">
        ...
        <div class="vv">
          <h1>驗證</h1>
          <!-- token： -->
          <input type="text" v-model="inputToken" class="form-control" placeholder="請輸入token" />
          <br />
        ...
        <div class="vv">
          ...
          <div class="todo" v-for="(item, index) in showTodores" :key="index">
            ...
            />
            <br />
            <button class="btn btn-warning" type="button" @click="editTodo(index)" v-if="editBut">
              修改</button
            >　
            ...
            <button class="btn btn-danger" type="button" @click="delTodo(item.id, index)" v-else>
              刪除
            </button>
          </div>
        </div>
...
const signIn = async () => {
  try {
    ...
    inputToken.value = res.data.token
